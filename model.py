...
def train():
    ...
    for epoch in range(config.num_epochs):
        ...
        for sequences, labels, pub_sequences in train_loader:
            sequences = config.tokenizer(sequences, padding=True, truncation=True, return_tensors='pt',
                                         max_length=config.max_length)
            sequences = {k: v.to(config.device) for k, v in sequences.items()}
            ...
            optimizer.zero_grad()
            logits = classifier(input_ids=sequences['input_ids'],
        ...
        f1_score = metrics.f1_score(train_all_labels, train_all_preds)
        mcc = matthews_corrcoef(train_all_labels, train_all_preds)
        torch.save(classifier.state_dict(), config.save_path)
        print(f"Epoch [{epoch + 1}/{config.num_epochs}] - Avg Loss: {avg_loss:.4f} - Accuracy: {train_accuracy:.4f}"
              f" - f1_score: {f1_score:.4f} - MCC: {mcc:.4f} - flag: {flag} - time: {time_dif}")
        test_mcc = test()
        print(f'test_mcc:{test_mcc} - best_mcc:{best_mcc}')
        if best_mcc < test_mcc:
            best_mcc = test_mcc
            torch.save(classifier.state_dict(), config.best_model)
...
def test():
    ...
    all_probs = []
    with torch.no_grad():
        classifier.load_state_dict(torch.load(config.save_path))
